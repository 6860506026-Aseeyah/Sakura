...
<script>
    ...
    let root = null;
    // ดึงข้อมูลเก่าที่เคยเซฟไว้ในเครื่อง (ถ้ามี)
    let nodesArray = JSON.parse(localStorage.getItem('sakuraTreeNodes')) || [];

    const canvas = document.getElementById('treeCanvas');
    const ctx = canvas.getContext('2d');

    // เมื่อเปิดหน้าเว็บ หรือ Refresh ให้วาดต้นไม้คืนมาทันที
    ...
    function addNode() {
        ...
        // ป้องกันเลขซ้ำเพื่อไม่ให้โครงสร้างสับสน
        ...
        // เก็บลงอาเรย์และบันทึกเข้า LocalStorage
        nodesArray.push(val);
    ...
    // ฟังก์ชันล้างข้อมูลแบบกดปุ๊บลบปั๊บ
    function resetTree() {
        root = null;
        nodesArray = [];
