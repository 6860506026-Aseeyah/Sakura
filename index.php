...
<div class="container">
    ...
    <div class="info-grid">
        ...
        <div class="guide-panel">
            ...
            <?php
                // ตัวอย่างการใช้ PHP สร้างลิสต์รายการ
                $methods = [
                ...
                foreach ($methods as $name => $path) {
                    echo "<div class='guide-item'><span class='dot'></span> <strong>$name:</strong> $path</div>";
                }
            ?>
        </div>
    </div>
...
<script>
    // Logic JavaScript ทั้งหมดคงเดิมเพื่อให้กราฟิกและ Interactive ทำงานได้บน Browser
    class Node {
        constructor(val) {
            this.val = val;
            this.left = null;
    ...
    let root = null;
    let nodesArray = JSON.parse(localStorage.getItem('sakuraTreeNodes')) || [];
    ...
    function addNode() {
        ...
        if (isNaN(val)) return;
        if (nodesArray.includes(val)) {
            alert("เลขนี้ปลูกไปแล้วจ้า!");
            input.value = '';
            return;
        }
        nodesArray.push(val);
        localStorage.setItem('sakuraTreeNodes', JSON.stringify(nodesArray));
        buildTreeFromValue(val);
        input.value = '';
        render();
    }
    ...
    function resetTree() {
        root = null;
        nodesArray = [];
        localStorage.removeItem('sakuraTreeNodes'); 
        render();
    }
</script>
