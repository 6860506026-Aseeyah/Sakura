...
<head>
    ...
    <style>
        ...
        /* ปรับส่วนฟอร์มให้เหลือกล่องเดียว */
        .form-group { display: flex; justify-content: center; align-items: center; gap: 10px; margin-bottom: 20px; }
        ...
        .output-row, .traversal-info { margin: 12px 0; display: flex; align-items: center; gap: 12px; }
...
<div class="container">
    ...
    <form method="POST" class="form-group">
        <input type="number" name="common_val" id="common_input" placeholder="ใส่ตัวเลข">
        <button type="submit" name="action_insert" class="btn btn-insert">ปลูก Node</button>
    ...
    <div class="bottom-panels">
        <div class="panel">
            ...
            <div class="output-row"><span class="label-text">Preorder:</span><span id="preText" class="value-text">-</span></div>
            <div class="output-row"><span class="label-text">Inorder:</span><span id="inText" class="value-text">-</span></div>
            <div class="output-row"><span class="label-text">Postorder:</span><span id="postText" class="value-text">-</span></div>
        </div>

        <div class="panel">
            <h3>3 วิธีหลักในการ Traversal:</h3>
            <div class="traversal-info">
                <span class="dot"></span><span class="label-text">Preorder:</span>
                <span class="formula-text">Root → Left → Right</span>
            </div>
            <div class="traversal-info">
                <span class="dot"></span><span class="label-text">Inorder:</span>
                <span class="formula-text">Left → Root → Right</span>
            </div>
            <div class="traversal-info">
                <span class="dot"></span><span class="label-text">Postorder:</span>
...
<script>
    ...
    nodesFromDB.forEach(val => { if (!root) root = new Node(val); else insertNode(root, val); });
    ...
    function draw() { ctx.clearRect(0, 0, canvas.width, canvas.height); if (root) drawNode(root, canvas.width / 2, 50, 160); updateText(); }
    function drawNode(node, x, y, offset) {
        if (node.left) { ctx.beginPath(); ctx.moveTo(x, y); ctx.lineTo(x - offset, y + 70); ctx.strokeStyle = '#ffb6c1'; ctx.stroke(); drawNode(node.left, x - offset, y + 70, offset / 1.8); }
        if (node.right) { ctx.beginPath(); ctx.moveTo(x, y); ctx.lineTo(x + offset, y + 70); ctx.strokeStyle = '#ffb6c1'; ctx.stroke(); drawNode(node.right, x + offset, y + 70, offset / 1.8); }
        ctx.beginPath(); ctx.arc(x, y, 18, 0, Math.PI * 2); ctx.fillStyle = "white"; ctx.fill(); ctx.strokeStyle = "#ff69b4"; ctx.lineWidth = 2; ctx.stroke();
